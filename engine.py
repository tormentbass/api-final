...
def carregar_componentes():
    ...
    print("\n========== [ENGINE] CARREGANDO MODELO JSON ==========")
    
    try:
        # 1. Carregar lista de colunas (features) e dados históricos
        ...
        # 2. Carregar o Booster do XGBoost (Formato JSON)
        if os.path.exists(MODEL_PATH):
            # Para arquivos .json, usamos o Booster nativo que é mais estável
            model = xgb.Booster()
            model.load_model(MODEL_PATH)
            print("✅ [ENGINE] Modelo JSON carregado com sucesso!")
        else:
            print("❌ [ENGINE] Arquivo JSON não encontrado!")
        
        return True if model else False
    except Exception as e:
        print(f"❌ [ENGINE] Erro no carregamento: {e}")
        return False

def get_latest_features(time_casa, time_fora, data_ref):
    if df_historico is None: return {"erro": "Sem histórico."}
    ...
    def get_last_row(team):
        ...
        return (h.iloc[0], "home") if h.iloc[0]["match_date"] >= a.iloc[0]["match_date"] else (away.iloc[0], "away")

    row_c, pref_c = get_last_row(time_casa)
    row_f, pref_f = get_last_row(time_fora)

    if row_c is None or row_f is None:
        return {"erro": f"Times não encontrados: {time_casa}/{time_fora}"}
    ...
    # Criamos o DMatrix, que é o formato que o modelo JSON exige
    df_ret = pd.DataFrame([f_dict])[features_finais].fillna(0)
    dmatrix = xgb.DMatrix(df_ret)
...
def gerar_relatorio_json(time_casa, time_fora, data_ref):
    if model is None: return {"status": "ERRO", "mensagem": "Modelo Offline"}
    
    feat = get_latest_features(time_casa, time_fora, data_ref)
    if "erro" in feat: return {"status": "ERRO", "mensagem": feat["erro"]}

    try:
        # Previsão usando o Booster
        probs = model.predict(feat["dmatrix"])[0]
